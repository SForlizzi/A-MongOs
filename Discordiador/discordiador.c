...
int flag_testeo = TEST_SERIALIZACION;
...
int main() {
	if(flag_testeo != DISCORDIADOR)
		correr_tests(flag_testeo);
	else {
...
t_list* lista_tripulantes_patota(uint32_t pid){
    ...
    while(respuesta->codigo_operacion != EXITO){
        list_add(lista_tripulantes_patota, list_remove(respuesta->lista, 0));
        //free(respuesta->tcb);
        //free(respuesta);
    ...
    while(un_tripulante->estado_tripulante != estado_tripulante[EXIT]){
        while(un_tripulante->estado_tripulante == estado_tripulante[EXEC]){
            ...
            if(tarea->codigo_operacion == TAREA){
            	t_tarea* una_tarea = (t_tarea*) remove_list(tarea->lista, 0);
                un_tripulante->tarea =  *una_tarea;
            }
...
void enlistarse(t_tripulante* un_tripulante){
    ...
    if(respuesta->codigo_operacion == TAREA){
    	t_tarea* una_tarea = (t_tarea*) remove_list(respuesta->lista, 0);
        un_tripulante->tarea = *una_tarea;
