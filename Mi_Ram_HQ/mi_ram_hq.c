...
void atender_clientes(void* param) {
	...
	while(flag) {
		...
		switch(mensaje_recibido->codigo_operacion) {
			...
			case PEDIR_TAREA:
				//tid = (int*) list_remove(mensaje_recibido->lista, 0);
				log_info(logger, "Pedido de tarea recibido, tid: %i\n", *tid);

				tid = (int*) list_remove(mensaje_recibido->lista, 0);
				t_tarea* una_tarea = buscar_siguiente_tarea(*tid);
			...
			case RECIBIR_TCB:
				...
				tcb = (t_TCB*) list_remove(mensaje_recibido, 0);
				gestionar_tcb(tcb);
				break;
			...
			case T_SIGKILL:
				...
				tid = list_remove(mensaje_recibido->lista, 0);
				if(eliminar_tcb(*tid)) {
					enviar_codigo(EXITO, parametros->socket);
					log_info(logger, "%i -KILLED", *tid);
				}
				else{
					enviar_codigo(FALLO, parametros->socket);
				}
			...
			case LISTAR_POR_PID:
				...
				pid = (int*) list_remove(mensaje_recibido->lista, 0);
				tcbs_de_esta_patota = buscar_tcbs_por_pid(*pid);
